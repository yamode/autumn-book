...
YAMADO 公式アプリ（yamado-one）の運用画面を `/admin` に増設した。**設計書: `docs/ADMIN_APP_OPS.md`**（文言・SQL・アラートしきい値の根拠はすべてここ）。

- 新規4画面: `/admin/app`（アプリ運用ダッシュボード）／`/admin/push`（アプリ通知）／`/admin/coupons`（クーポン）／`/admin/preferences`（好み登録項目）
- 管理メニューを **17項目フラット → 4グループ21項目**に再編（宿泊・直販／アプリ・会員／サイト・コンテンツ／システム）。URL は一切変えていない。テナント横断の画面では施設セレクトを「全施設共通」に差し替える
- **書き込みはすべて `book.admin_*` の SECURITY DEFINER RPC 経由**。service_role キーは持ち込まない（権限判定・監査ログを DB 側で完結させ、`AUTH_MODE=demo` の偽造 cookie で突破されない設計にする）
- 新規画面は**実データ専用**。`ADMIN_SUPABASE`（= DATA_SOURCE と AUTH_MODE が両方 supabase）が false のときは「本番データ専用です」パネルのみ表示する
- **通知の誤配信対策**: テスト送信（@yamado.co.jp の会員）を済ませないと本番送信ボタンが押せない。全会員送信は宛先数の手入力で確認。即時配信は取り消し不可（2分で drain されるため）、予約配信のみ取り消せる
- `/admin/news` の公開済み記事に「アプリに通知する →」（`/admin/push/new` へプリフィル）を追加
- **DB migration は `autumn-shared/supabase/migrations/20260907061853_book_admin_app_ops.sql`**（RPC 15本 + `book.notification_campaigns` テーブル + RLS）。`coupons_staff_write`（for all）を select のみに絞る変更を含む
- 新しい管理者を増やすときは **`app_metadata.role='admin'` と `core.memberships` の `tenant_admin` の両方**が必要（片方だけだと画面は開けるが RPC が `forbidden` になる）
- **未実装（次タスク）**: `/admin/members` の実データ化と会員詳細のタブ（クーポン／通知／端末／好み）。現状 `/admin/members` は本番でも demo 会員を表示している
