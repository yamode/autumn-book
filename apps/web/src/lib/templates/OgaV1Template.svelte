...
<div class="oga-v1 bg-white" style="--fac-heading: #1c1c1c; --fac-accent: #555;">
	...
	<section class="relative">
		...
		<div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white">
			<p class="text-xs tracking-[0.6em] opacity-80">OGA PENINSULA, AKITA</p>
			<h1 class="mt-4 text-3xl font-light tracking-[0.4em] sm:text-5xl">{f.name}</h1>
	...
	<div class="mx-auto max-w-6xl space-y-24 px-4 py-20">
		...
		<section class="mx-auto max-w-2xl text-center">
			...
			<p class="mt-8 text-sm leading-[2.4] tracking-wide text-stone-500">{f.description}</p>
		</section>

		<!-- 3本柱: NATURE / CUISINE / STAY（左右交互の大判写真） -->
		{#each pillars as pillar, i}
			<section class="grid items-center gap-8 md:grid-cols-2 {i % 2 === 1 ? 'md:[&>*:first-child]:order-2' : ''}">
				...
				<div class="px-2 md:px-8">
					<h2 class="text-sm font-semibold tracking-[0.5em] text-stone-400">{pillar.key}</h2>
		...
		<section>
			<h2 class="mb-8 text-center text-sm font-semibold tracking-[0.5em] text-stone-400">ABOUT</h2>
		...
		<!-- プラン -->
		<section>
			<h2 class="mb-8 text-center text-sm font-semibold tracking-[0.5em] text-stone-400">PLAN</h2>
			<div class="space-y-4">
