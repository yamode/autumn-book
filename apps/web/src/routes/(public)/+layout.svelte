	...
	<header class="sticky top-0 z-50 border-b border-stone-200 bg-white/95 backdrop-blur">
		<div class="mx-auto flex max-w-6xl flex-wrap items-center gap-x-6 gap-y-2 px-4 py-2.5">
			...
			<nav class="flex items-center gap-4 text-sm text-stone-600">
				<a href="/search" class="hover:text-brand-800">宿を探す</a>
				<a href="/yamado/nishiwaga" class="hidden hover:text-brand-800 sm:inline">西和賀</a>
				<a href="/yamado/oga" class="hidden hover:text-brand-800 sm:inline">男鹿</a>
			</nav>
			<div class="ml-auto flex items-center gap-3 text-sm">
				{#if data.user?.role === 'member'}
					<a href="/account" class="font-medium text-brand-800 hover:underline">{data.user.name} 様</a>
				{:else}
					<a href="/auth/login" class="text-stone-600 hover:text-brand-800">ログイン</a>
					<a href="/auth/register" class="rounded-md bg-accent-600 px-3 py-1.5 text-white hover:bg-accent-500">会員登録</a>
				{/if}
			</div>
	...
	<footer class="mt-16 border-t border-stone-200 bg-brand-900 text-stone-300">
		<div class="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:grid-cols-3">
			<div>
				...
				<p class="mt-2 text-xs leading-relaxed">東北の山と海に、土地の記憶を味わう小さな宿を。</p>
			</div>
			<div class="text-sm">
				<p class="mb-2 font-medium text-white">施設</p>
				<ul class="space-y-1 text-xs">
					<li><a href="/yamado/nishiwaga" class="hover:underline">山人 -yamado-（岩手・西和賀）</a></li>
					<li><a href="/yamado/oga" class="hover:underline">山人 -oga-（秋田・男鹿）</a></li>
				</ul>
			</div>
			<div class="text-sm">
				<p class="mb-2 font-medium text-white">ご案内</p>
				<ul class="space-y-1 text-xs">
					<li><a href="/legal/tokushoho" class="hover:underline">特定商取引法に基づく表記</a></li>
					<li><a href="/legal/privacy" class="hover:underline">プライバシーポリシー</a></li>
					<li><a href="/legal/yakkan" class="hover:underline">宿泊約款</a></li>
					<li><a href="/admin" class="text-stone-500 hover:underline">施設管理者の方</a></li>
				</ul>
			</div>
		</div>
		<p class="border-t border-white/10 py-3 text-center text-[11px] text-stone-500">
			© 株式会社山人 ｜ デモ環境（料金・空室・写真はすべてサンプルです）
		</p>
	</footer>
</div>
