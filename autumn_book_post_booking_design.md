...
| 8 | **admin 系（`/admin/options`・`/admin/cancel-policies`）が Supabase 未接続**（本アプリは AUTH_MODE=supabase 前まで admin 全体が demo 運用）。本番では P1 seed が `is_active=false`・P3 seed が `rules='[]'` のまま | admin の Supabase Auth 実接続（全 admin 共通の別課題）まで保留。**公開前に本番 DB へ直接投入が必要**: ①オプションの `is_active=true`・単価 ②キャンセル規定 `rank_cancel_policies.rules`（standard 行＝基準規定は公開前必須）。いずれも service_role で SQL 投入。`sbListBookingOptionsAdmin` / service_role 書込は接続用に用意済み |
