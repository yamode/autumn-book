...
- 押さえが立っている日は **宿泊のある日**（男鹿 525日中509日・西和賀 157日中151日）＝
  営業日に毎回 19:00〜20:30 を塞いでいた運用。

**そのまま公開すると、夕食後のいちばん取りたい時間が両施設とも「満」で固定表示される。**

→ **同じルールをデータではなく設定で表した**（時間帯を夕食の前後2本に分ける）。
押さえ行が1件も無くても空き状況は同一になる。

| 作業 | 結果 |
|---|---|
| 男鹿の浴室を登録 | `San♨️`（1室） |
| 押さえ枠を削除 | 男鹿 1,575 / 西和賀 471 ＝ **2,046件**。本物の予約4件は残した |
| 残った行の `bath_id` を補完 | 4件すべて（Blocker B2 の解消） |
| FM の実予約を stay へ紐付け | 2件（部屋404→岡部様・部屋202→星野様）。**夕方の枠は「その夜の予約」**として照合し、どちらも候補1件に確定 |
| 時間帯の設定 | 両施設とも `15:00–19:00` ＋ `20:30–22:30` ＋ 翌朝。西和賀は使われていなかった施設既定（朝だけ）も同じ形に直した |

PROD の `book._pb_slots` で検証した結果（夕方12枠・19:00〜20:30 は出ない）:

```
男鹿 San♨️  夕 15:00 15:30 16:00 16:30 17:00 17:30 18:00 18:30 20:30 21:00 21:30 22:00 ／ 朝 06:00〜09:30（8枠）
西和賀 一寸 夕 15:00 15:30 16:00 16:30 17:00 17:30 18:00 18:30 20:30 21:00 21:30 22:00 ／ 朝 05:30〜10:00（10枠）
```
...
- 2026-08-31 段階0を実施（§5 冒頭）。FM 由来の押さえ枠 2,046件を設定へ置き換えて削除。浴室マスタ整備・bath_id 補完・実予約2件の紐付け。
