...
### 段階0 の実施記録（2026-08-31 完了）

着手して分かった **当初の想定と最も違った点**: `pms.private_bath_slots` の 2,050行のうち
**2,046行は予約ではなく「FileMaker で毎日 × を付けていた押さえ枠」**だった。

- 両施設とも **毎日 19:00 / 19:30 / 20:00 の3枠だけ**。部屋番号なし・氏名なし・予約に紐づかない。
  男鹿は 2025-05-07〜2027-11-24 の525日、西和賀は157日ぶん。
- 根拠は取り込みスクリプト自身のコメント（`autumn-pms/migration/import_fm_extras.py:287`）:
  「`is_empty=0`（FM 上で × ＝押さえ済み）の枠だけ取り込む」。
  部屋番号が `note` に入るのは客に割り当てた枠なので、**部屋番号なし＝誰にも割り当てていない押さえ**。
- 押さえが立っている日は宿泊のある日（男鹿 525日中509日・西和賀 157日中151日）。
- **理由は夕食時間**（オーナー確認）。ただし **その運用は現在は行っていない**（同・2026-08-31）。

→ つまりこの 2,046行は **すでに効力を失った押さえがデータとして残り続けていたもの**。
そのまま公開していれば、夕食後のいちばん取りたい時間が両施設とも「満」で固定表示された。
**削除がそのまま正解**で、時間帯の設定は従来どおり（夕方は 15:00–22:30 の1本）でよい。

> ⚠ 作業の途中で「夕食時間は今も出していない」と誤って解釈し、
> 時間帯を `15:00–19:00` ＋ `20:30–22:30` の2本に変更した。
> 運用が死んでいると分かった時点で **元の1本（15:00–22:30）へ戻した**（同日中）。
> 死んだ運用ルールを設定に焼き込まない、が正しい。

| 作業 | 結果 |
|---|---|
| 男鹿の浴室を登録 | `San♨️`（1室） |
| 効力を失った押さえ枠を削除 | 男鹿 1,575 / 西和賀 471 ＝ **2,046件**。本物の予約4件は残した |
| 残った行の `bath_id` を補完 | 4件すべて（Blocker B2 の解消） |
| FM の実予約を stay へ紐付け | 2件（部屋404→岡部様・部屋202→星野様）。**夕方の枠は「その夜の予約」**として照合し、どちらも候補1件に確定 |
| 時間帯の設定 | **変更なし**（夕方 15:00–22:30 ＋ 翌朝）。西和賀の使われていない施設既定が朝だけになっていたのは同じ形に直した |

PROD の `book._pb_slots` で検証した最終状態:

```
男鹿 San♨️  夕 15:00〜22:00（15枠）／ 朝 06:00〜09:30（8枠）   台帳 0件
西和賀 一寸 夕 15:00〜22:00（15枠）／ 朝 05:30〜10:00（10枠）  台帳 4件
```

**台帳は本物の予約4件だけになった**（西和賀のみ・すべて `bath_id` あり・すべて stay 紐付け済み）。

残る 0-3 / 0-4:

- `pms.items` の「貸切風呂」登録は**まだ不要**。設定に `itemId` を入れなければ、
  品名「貸切風呂」・税率10%・部門なしで明細が立つ（部門を分けたくなった時点で登録すればよい）。
- `booking.yamado.co.jp` の DNS / Cloudflare Pages カスタムドメインは**未設定**。館内図に QR を刷る前に必須。

**受付は無効・料金0円のまま**。男鹿の 2,200円 を入れると、その時点から
スタッフが入れた枠にも金額が焼かれ請求明細が立つので、公開の準備が整ってから入れる。
...
| book 管理画面 | `/admin/bath`（予約フォームの文章・写真の編集 ＋ QR テスト） | **実装済** |
| 館内図 QR | `MapBlock` に `kind:'qr'` ＋ トークン get-or-create の呼び出し | **未着手（意図的に最後へ）** |
| 段階0 | 浴室マスタ整備・押さえ枠の設定化・bath_id 補完・実予約の紐付け | **完了**（`pms.items` とドメインは残） |
...
### 管理画面（/admin/bath・2026-09-01 追加）

館内図に刷る前に、**文言と QR を book 側だけで詰め切れる**ようにしたもの。

| 何を | どう |
|---|---|
| 文章 | 見出し・説明文・料金の注記・注意事項・完了メッセージの5欄を言語ごとに（ja / en / zh-TW）。**未入力の欄はアプリの既定文言がそのまま出る**ので、空のまま保存しても画面は壊れない |
| 写真 | 最大12枚・並べ替え・削除。既存の `book-photos` バケットを再利用。**押した時点で保存**する（持ち帰って消えない） |
| 言語のフォールバック | 読み出しは ja を土台に、その言語の**空でない欄だけ**上書き。翻訳が途中でも虫食いにならない |
| QR テスト | その日に泊まっているお客様を選ぶと、トークンを取り出して（無ければ発行して）QR を表示。スマホで読めば実際に `/r/bath` まで通る。get-or-create なので何度押しても同じ QR |
| 保存の安全側 | `DATA_SOURCE=supabase` かつ `AUTH_MODE=supabase` のときだけ通す。そうでなければ**黙って成功させず**理由を出す（v0.32.1 で踏んだ「保存が初期値に戻る」を繰り返さない） |
| 権限 | admin / staff のみ。RPC 側でも `has_facility_access` で二重に守る |

本番は `wrangler.jsonc` の `vars` が `AUTH_MODE=supabase` / `DATA_SOURCE=supabase` なので、
そのまま実データへ保存できる（`/admin/inroom` のコメントにある「現状 demo」はローカル `.env` の話）。

**QR の描画**: `$lib/qr.ts` ＋ `QrCode.svelte`。`viewBox` だけを持つ SVG にして、
大きさは呼び出し側が決める（館内図は用紙・レイアウトで原寸が変わるため）。
既存の印刷スリップ（`/admin/inroom/slip/[id]`）は同じライブラリを
サーバ側で `createSvgTag`（cellSize を px で焼く）で使っており、**実装が2つある**。
スリップを刷り直すときにこちらへ寄せる。
...
- 2026-09-01 book 管理画面 `/admin/bath`（予約フォームの文章・写真の編集 ＋ 実際の予約での QR テスト）。migration `20260831234706_book_private_bath_contents`。
- 2026-08-31 段階0を実施（§5 冒頭）。効力を失った FM 由来の押さえ枠 2,046件を削除。浴室マスタ整備・bath_id 補完・実予約2件の紐付け。時間帯の設定は従来どおり。
